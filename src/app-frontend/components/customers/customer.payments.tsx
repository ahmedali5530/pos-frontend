...
export const CustomerPayments: FC<Props> = ({
  ...
  const loadFreshCustomer = async () => {
    const c = await customerHook.fetchCustomer(customerProp.id);
  ...
  const createPayment = async (values: any) => {
    ...
    try {
      if (values.order_id) {
        values.order = toRecordId(values.order_id.value);
      }

      values.amount = Number(values.amount);


      const [payment] = await db.insert(Tables.customer_payment, {
        ...values,
        customer: toRecordId(customer?.id)
      });

      await db.merge(toRecordId(customer?.id), {
        payments: Array.from(new Set([...customer?.payments.map(item => toRecordId(item.id)) ?? [], payment.id]))
      });
  ...
  const diff = useMemo(() => {
    ...
    return customerHook.calculateCustomerSale(customer) - customerHook.calculateCustomerPayment(customer) + Number(customer.opening_balance ?? 0);
  }, [customer]);
        ...
        >
                  ...
                  render={(props) => (
                    <ReactSelect
                      options={customer?.orders?.reverse().map(item => {
                        return {
                          label: `${item.order_id} (${DateTime.fromJSDate(item.created_at).toFormat(
          ...
          <div className="grid grid-cols-5 gap-3 mb-5">
            <div className="border border-primary-500 p-5 font-bold text-primary-500 rounded">
              OP Balance
              <span className="float-right">{withCurrency(customer?.opening_balance)}</span>
            </div>
            <div className="border border-primary-500 p-5 font-bold text-primary-500 rounded">
              Credit Sale
              <span
                className="float-right">{withCurrency(customer ? customerHook.calculateCustomerSale(customer) : 0)}</span>
            </div>
            <div className="border border-success-500 p-5 font-bold text-success-900 bg-success-100 rounded">
              Payments
              <span
                className="float-right">{withCurrency(customer ? customerHook.calculateCustomerPayment(customer) : 0)}</span>
            </div>
            <div className={
              classNames(
            ...
            <div className={
              classNames(
                "border p-5 font-bold rounded",
                customer?.credit_limit && diff > 0 ? 'text-danger-900 bg-danger-100 border-danger-500' : 'text-primary-900 bg-primary-100 border-primary-500'
              )
            }>
              Credit limit
              <span
                className="float-right">{customer?.credit_limit ? withCurrency(customer?.credit_limit) : 'no limit'}</span>
            </div>
          </div>
          ...
          <table className="table border border-collapse">
            ...
            {list.map((item: any, index: number) => (
              <tr key={index} className="hover:bg-gray-100">
                <td
                  title={item.created_at}>{DateTime.fromJSDate(item.created_at).toFormat(import.meta.env.VITE_DATE_TIME_HUMAN_FORMAT)}</td>
                <td>
                  {item.amount && (
                    <>
                ...
                <td>
                  {item.order_id && (
                    <ViewOrder order={item}>
                      <FontAwesomeIcon icon={faEye} className="mr-2"/> {item.order_id}
                    </ViewOrder>
                  )}
                </td>
              </tr>
