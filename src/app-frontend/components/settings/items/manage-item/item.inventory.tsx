...
export const ItemInventory = ({
  ...
  const id = watch('id');
  const stores = id ? watch('stores') : watch('storesDropdown');
      ...
      <div className="grid grid-cols-4 gap-3 gap-y-2 mb-4">
        ...
        <div className="col-span-4">
          {stores?.map((store: any, index: number) => {
            const storeId = store.store || store.value;
            return (
              <div className="bg-gray-100 rounded p-3 mb-3" key={index}>
                <div className="grid grid-cols-4 mb-3 gap-3 hover:bg-gray-200">
                  <div>
                    <input type="hidden" {...register(`stores.${index}.store`)} value={storeId}/>
                    {store.label}
                  </div>
