    ...
    const fetchData = async () => {
      try {
        ...
        if (filters.userIds.length > 0) {
          conditions.push(`purchased_by INSIDE $users`);
          params.users = filters.userIds.map(id => toRecordId(id));
        }

        // Note: supplier filtering is done on item level, not purchase level
        // We'll filter items after fetching
        ...
        const purchasesQuery = `
          ...
          FETCH items, items.item, items.item.categories, items.supplier, purchased_by, store, supplier
        `;

        const result: any = await queryRef.current(purchasesQuery, params);
        const purchases = (result?.[0] ?? []) as any[];
        ...
        purchases.forEach((purchase: any) => {
          ...
          items.forEach((item: any) => {
            ...
            // Filter by item-level supplier if specified
            if (filters.supplierIds.length > 0) {
              const itemSupplierId = typeof item.supplier?.id === "string"
                ? item.supplier.id
              ...
              const matchesSupplier = filters.supplierIds.some(filterId => {
                ...
                return itemSupplierId === normalizedFilterId;
              });
              if (!matchesSupplier) {
                return;
              }
            ...
            const price = safeNumber(item.purchase_price);
            const amount = quantity * price;

            allPurchaseItems.push({
              date: purchase.created_at || "",
              purchaseNumber: purchase.purchase_number ? `#${purchase.purchase_number}` : "N/A",
              supplier: item.supplier?.name || purchase.supplier_name || purchase.supplier?.name || "N/A",
              store: purchase.store_name || purchase.store?.name || "N/A",
              ...
              price: price,
              amount: amount,
              user: purchase.user_name || purchase.purchased_by?.name || "Unknown",
              comments: item.comments || undefined,
              paymentMode: purchase.purchase_mode || purchase.payment_type?.name || undefined,
            });
          });
        });
          ...
          <div className="p-4">
            <table className="min-w-full text-sm">
              <tbody className="divide-y divide-neutral-100">
                <tr>
                  <td className="py-2 text-neutral-700">Total Purchases</td>
                  <td className="py-2 text-right font-semibold text-neutral-900">{formatNumber(purchaseItems.length)}</td>
                </tr>
                <tr>
                ...
                {purchaseItems.length > 0 ? (
                  purchaseItems.map((item, index) => (
                    <tr key={`${item.date}-${item.item}-${index}`}>
                      ...
                      <td className="py-3 px-3 text-sm text-neutral-700">{item.purchaseNumber}</td>
